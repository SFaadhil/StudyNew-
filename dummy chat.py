...
2. Cite every fact like: (Source: filename, Page X)
3. For specific details — numbers, durations, names, timeframes:
   - Clearly stated in source → quote it exactly
...
6. FORMAT:
   - Write 2-3 paragraphs explaining what the source says
   - Follow with bullet points listing specific facts, rules, numbers, conditions
   - Cite every bullet point
...
RULES:
1. Only use facts from the source. Cite file + page for every point.
2. For numbers, durations, names, deadlines:
   - Clearly in source → quote exactly
...
def strict_agent(query: str, pages: list[dict]) -> dict:
    reply = call_ollama(STRICT_PROMPT.format(context=build_context(pages), query=query))
    return {"reply": reply, "sources": make_sources(pages), "raw_pages": make_raw(pages), "mode": "strict"}
...
class ChatPayload(BaseModel):
    message: str
    mode:    str = "strict"   # strict | expand | detail | kid | source_explain
...
def chat(payload: ChatPayload):
    ...
    if not pages:
        return {
            "reply":     "This topic isn't covered in your uploaded material. "
                         "Try ✨ Expand with AI for a general answer.",
            "sources":   [], "raw_pages": [], "mode": "not_found"
        }
